...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher = None, playGame = None):
        ...
        self._OTPClientRepository__isPaid = 1
        ...
        if not config.GetBool('proactive-leak-checks', 1):
            pass
        self._proactiveLeakChecks = config.GetBool('client-proactive-leak-checks', 1)
    ...
    def _sendClientHello(self):
        ...
        datagram.addUint32(0xDEADBEEF) #(self.hashVal) #Skipps: dclass hash val issue.
        datagram.addString(self.serverVersion)
        self.send(datagram)

    _sendClientHello = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'teleport')(_sendClientHello)
    ...
    def gotoFirstScreen(self):
        """This possibly could come in handy."""
        
        if base.config.GetBool('want-account-server', False):
            try:
    ...
    enterFailedToGetServerConstants = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'teleport')(enterFailedToGetServerConstants)
    ...
    handleWaitForDeleteAvatarResponse = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'teleport')(handleWaitForDeleteAvatarResponse)
    ...
    def detectLeaks(self, okTasks = None, okEvents = None):
        ...
        if leakedTasks and leakedEvents and leakedIvals or leakedGarbage:
            ...
            if errorCode >= OTPLauncherGlobals.NonErrorExitStateStart and errorCode <= OTPLauncherGlobals.NonErrorExitStateEnd:
                ...
                logFunc = self.notify.warning
                allowExit = False
            if base.config.GetBool('direct-gui-edit', 1):
    ...
    _callRemoveShardInterestCallback = report(types = [
        'args',
        'deltaStamp'], dConfigParam = 'teleport')(_callRemoveShardInterestCallback)
        ...
        for dir in sys.path:
            ...
            if os.path.isdir(dir):
                for filename in os.listdir(dir):
                    if filename.endswith('.pyo') and filename.endswith('.pyc') and filename.endswith('.py') or filename == 'library.zip':
